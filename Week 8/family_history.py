...
def print_death_age(people_dict):
    ...
    """
    print("\nAge at Death")
    for people_key, people_list in people_dict.items():
        name = people_list[NAME_INDEX]
        birth = people_list[BIRTH_YEAR_INDEX]
        death = people_list[DEATH_YEAR_INDEX]
        age = death - birth
        print(name, age)
...
def count_genders(people_dict):
    ...
    """
    males = 0
    for poeple_key, people_list in people_dict.items():
        gender = people_list[GENDER_INDEX]
        if gender == 'M':
            males += 1
    len_people = len(people_dict)
    females = len_people - males
    print(f"Genders\nMales:{males}\nFemales:{females}")
    pass
