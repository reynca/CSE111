...
def main():
    ...
    molecule = input("Please type the molecule that you want to test(Exp. glucose = C6H12O6): ")

    # Get the mass of a chemical sample in grams from the user.
    mass = float(input("Please type the mass of the sample size(g): "))
    ...
    symbol_quantity_list = parse_formula(molecule, periodic_table)
    ...
    num_moles = round(mass/molar_mass, 5)
...
def make_periodic_table():
    periodic_table_dict = {"Ac": ["Actinium", 227],"Ag": ["Silver", 107.8682],"Al": ["Aluminum",	26.9815386],
...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict):
    ...
    for item in symbol_quantity_list:
        sym = item[0]
        quan = item[1]
        if sym in periodic_table_dict:
            id = periodic_table_dict[sym]
            atomic_mass = id[ATOMIC_MASS_INDEX]
            mass += round(atomic_mass * quan, 5)    
